...
function initializePortfolio(){const fragment=document.createDocumentFragment();try{handleLoading();setupNavigation();setupScrollEffects();setupAnimations();setupContactForm();setupBackToTop();setupThemeHandling();const resumeModal=document.getElementById("resume-modal");if(resumeModal){setupResumeModal()}
...
observer.unobserve(img)}})},{rootMargin:'50px 0px',threshold:0.1});images.forEach(img=>{if(!img.hasAttribute('loading')){img.loading='lazy'}
img.decoding='async';if(img.closest('.hero')){img.fetchPriority='high'}
img.onerror=function(){console.warn('Failed to load image:',this.src);setTimeout(()=>{if(this.dataset.originalSrc){this.src=this.dataset.originalSrc}},1000)};if(img.src){img.dataset.originalSrc=img.src}
